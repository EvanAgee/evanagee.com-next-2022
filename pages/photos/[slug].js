    ...
    <>
        ...
        <header className="p-6 lg:p-16 lg:pb-0 font-display flex flex-col lg:flex-row lg:justify-between text-right lg:text-left lg:items-baseline gap-6">
          <div>
            <h1 className="text-3xl lg:text-6xl text-white mb-4">
              {photo?.title?.rendered && <WpApiContent content={photo?.title?.rendered} />}
            </h1>
          ...
          <div>
            <Link href="/photos">
              <a  className="button button-sm whitespace-nowrap">Back to Photos</a>
            </Link>
          </div>
        </header>
      </div>
      <div className="dark:bg-gray-900 dark:text-gray-300 p-6 pb-12 lg:p-16">
        <div className="grid lg:grid-cols-3 lg:gap-16">
          <div>
            {photo?.content?.rendered && <WpApiContent content={photo?.content?.rendered} />}
          </div>
      ...
      {catPosts &&
        catPosts.filter((c) => c.id !== photo.id).length > 0 && (
          <BadgeWrapper
...
export async function getStaticProps(context) {
  const photo = await fetch(`${settings.apiBase}/photos?slug=${context.params.slug}&per_page=1`);
  ...
  if (res[0]?.ea_photo_albums && res[0]?.ea_photo_albums?.length > 0) {
    const posts = await fetch(`${settings.apiBase}/photos?photo_album=${res[0]?.ea_photo_albums[0].term_id}&per_page=10`);
...
export async function getStaticPaths() {
  ...
  while (keepGoing) {
    const res = await fetch(`https://blog.evanagee.com/wp-json/wp/v2/photos?per_page=10&page=${page}`)
